...
<head>
  ...
  <title>Flowdesk Asset Reminder</title>
</head>
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #dbe4f0;">
    <tr>
      <td style="padding:24px 32px;background:#0f2747;color:#ffffff;">
        ...
        <div style="font-size:14px;opacity:0.9;margin-top:6px;">Asset reminder</div>
      </td>
    </tr>
    <tr>
      <td style="padding:32px;">
        ...
        <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">
          This is a Flowdesk reminder about an asset update.
        </p>
          ...
          <tr>
            <td style="padding:20px;">
              <div style="font-size:14px;line-height:1.8;">
                <div><strong>Asset:</strong> {{ $log->asset?->name ?? 'Asset' }}</div>
                <div><strong>Code:</strong> {{ $log->asset?->asset_code ?? 'N/A' }}</div>
                <div><strong>Event:</strong> {{ \Illuminate\Support\Str::of((string) $log->event)->replace('.', ' ')->headline() }}</div>
                <div><strong>Reference Date:</strong> {{ (string) (($log->metadata['due_date'] ?? $log->metadata['event_date'] ?? 'N/A')) }}</div>
              </div>
            </td>
          </tr>
        </table>
        ...
        <p style="margin:0;font-size:13px;line-height:1.7;color:#64748b;">
          Please review this asset in Flowdesk and take action if needed.
        </p>
      </td>
    </tr>
  </table>
