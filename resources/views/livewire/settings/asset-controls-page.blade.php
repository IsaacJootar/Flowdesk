<div class="space-y-6">
    ...
    >
        @if ($feedbackMessage)
            <div
                wire:key="asset-controls-feedback-success-{{ $feedbackKey }}"
                x-data="{ show: true }"
    ...
    <div class="fd-card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
                <span class="inline-flex items-center rounded-full border border-slate-300 bg-slate-200 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-800">
                    Asset Permission Matrix
                </span>
                <h2 class="mt-2 text-base font-semibold text-slate-900">Per-Action Role Access for Asset Operations</h2>
                <p class="mt-1 text-sm text-slate-600">
                    Choose exactly which roles can perform each asset action from registry visibility to disposal.
                </p>
            </div>
        ...
        <form wire:submit.prevent="save" class="space-y-4">
            ...
            <div class="flex justify-end">
                ...
                >
                    <span wire:loading.remove wire:target="save">Save Asset Controls</span>
