...
class RequestsPage extends Component
{
    ...
    public function addThreadComment(AddRequestComment $addRequestComment): void
    {
        ...
        try {
            $addRequestComment(auth()->user(), $request, [
    ...
    public function saveDraft(CreateRequestDraft $createRequestDraft, UpdateRequestDraft $updateRequestDraft): void
    {
        $this->feedbackError = null;
        ...
        try {
            if ($this->isEditing && $this->editingRequestId) {
                ...
                $updateRequestDraft(auth()->user(), $request, $payload);
                $this->setFeedback('Request draft updated.');
            } else {
                $createRequestDraft(auth()->user(), $payload);
    ...
    public function render(): View
    {
        ...
        $requests = $this->readyToLoad
            ? $this->requestQuery()->paginate($this->perPage)
    ...
    private function decideSelectedRequest(DecideSpendRequest $decideSpendRequest, string $action): void
    {
        ...
        try {
            $updated = $decideSpendRequest(auth()->user(), $request, [
                'action' => $action,
    ...
    {
        $user = auth()->user();
        $approvableIds = $this->approvableRequestIds();
        $actedRequestIds = $this->actedOnRequestIds();
        $decidedByMeIds = $this->decidedByMeRequestIds();
    ...
    {
        return strtoupper((string) (auth()->user()?->company?->currency_code ?: 'NGN'));
    }
    ...
    {
        $user = auth()->user()?->loadMissing('department:id,name');
