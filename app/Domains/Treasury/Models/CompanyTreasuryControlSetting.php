...
class CompanyTreasuryControlSetting extends Model
{
    ...
    public static function defaultControls(): array
    {
        ...
        return [
            ...
            'reconciliation_backlog_alert_count_threshold' => max(1, (int) ($defaults['reconciliation_backlog_alert_count_threshold'] ?? 5)),
            'exception_action_allowed_roles' => array_values(array_filter(array_map(
                static fn (mixed $role): string => strtolower(trim((string) $role)),
                (array) ($defaults['exception_action_allowed_roles'] ?? ['owner', 'finance'])
            ))),
            'exception_action_requires_maker_checker' => (bool) ($defaults['exception_action_requires_maker_checker'] ?? true),
